    ...
    <Card style={{ marginBottom: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1.5rem' }}>
        <div>
          ...
          <p style={{ fontSize: '1.5rem', fontWeight: '800' }}>₹{credit.creditLimit?.toLocaleString()}</p>
        </div>
        <div>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Utilized</p>
          <p style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--error)' }}>₹{credit.usedCredit?.toLocaleString()}</p>
        </div>
        <div>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Available</p>
          <p style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--success)' }}>₹{credit.availableCredit?.toLocaleString()}</p>
        </div>
        <div>
